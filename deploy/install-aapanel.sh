#!/usr/bin/env bash
#
# One-shot installer for a VPS already running aaPanel.
#
#   sudo DOMAIN=hrms.company.internal ./deploy/install-aapanel.sh
#
# Unlike install.sh, this does NOT install Caddy or bind anything to 80/443 —
# aaPanel's own Nginx already owns those ports. It only sets up the backend
# and frontend as systemd services on loopback ports; you point an aaPanel
# site + reverse proxy + SSL at them afterwards (see deploy/README.md).
#
...
[[ -n "${DOMAIN:-}" ]] || die "set DOMAIN, e.g. DOMAIN=hrms.company.internal sudo -E ./deploy/install-aapanel.sh"

log "Checking DNS for $DOMAIN"
resolved="$(getent hosts "$DOMAIN" | awk '{print $1}' | head -1 || true)"
if [[ -z "$resolved" ]]; then
  warn "$DOMAIN does not resolve from this box yet. aaPanel's Let's Encrypt issuance will fail until it does."
else
  echo "    $DOMAIN -> $resolved"
fi

if ss -ltnp 2>/dev/null | grep -q ':80 '; then
  echo "    port 80 is already in use (expected — that should be aaPanel's Nginx)"
fi
...
# Browser-visible: same origin, aaPanel's Nginx strips /api before proxying to FastAPI.
...
printf '\n'
if [[ $failed -eq 0 ]]; then
  log "Backend and frontend are up on loopback"
  echo "    Next: in aaPanel, create a site for $DOMAIN, paste"
  echo "    deploy/aapanel-nginx.conf.snippet into its Nginx config, and issue"
  echo "    a free SSL cert from the site's SSL tab. See deploy/README.md."
else
